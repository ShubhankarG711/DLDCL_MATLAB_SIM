function [xi_Est,s_new,L_zi_new,Lpr_zi_new] = DLDCL1hop(t, s_old, N, G, rbt_ID, xj_Act, xj_Prj, xj_IMU, xTg, mu_dx,L_wij,L_zi_prev,Lpr_wij,Lpr_zi_prev)
% DL-DCL algorithm for 2-D position
...
% RPSS estimates 
for j = 1:N
    d_jH(:,j) = RangeSensing(xj_Act(:,j),xTg,mu_dx(j));
...
what_ij = exp(-eta_w.*Lwij_prev.*s_old);
wphat_ij = exp(-eta_w.*Lwijpr_prev.*s_old);
zihat = exp(-eta_z.*L_zi_prev.*s_old);
ziprhat = exp(-eta_z.*Lpr_zi_prev.*s_old);
...
nj = 1;
for j = Neigh_ID
   xtilde_jH(:,nj) = xj_IMU(:,j)+d_jH(:,j);
   xhat_jH(:,nj) = xj_Prj(:,j)+d_jH(:,j);
   nj = nj+1;
end
xtilde_H = sum(repmat(w_ij,length(xTg),1).*xtilde_jH,2);
xhat_H = sum(repmat(wp_ij,length(xTg),1).*xhat_jH,2);

xH_Est = zi*xtilde_H + (1-zi)*xhat_H;
...
nj = 1;
for j = Neigh_ID
   xtilde_ji(:,nj) = xj_IMU(:,j)+d_ji(:,j);
   xhat_ji(:,nj) = xj_Prj(:,j)+d_ji(:,j);
   nj = nj+1;
end
xtilde_i = sum(repmat(w_ij,length(xTg),1).*xtilde_ji,2);
xhat_i = sum(repmat(wp_ij,length(xTg),1).*xhat_ji,2);

xi_Est = zi*xtilde_i + (1-zi)*xhat_i;
